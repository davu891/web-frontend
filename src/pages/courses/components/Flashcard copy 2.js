...
const Flashcard = ({ vocabData }) => {
    const [isFlipped, setIsFlipped] = useState(false);
    const [currentCard, setCurrentCard] = useState(0);
    const [isShuffled, setIsShuffled] = useState(false);
    const [isReversed, setIsReversed] = useState(false);
    const [displayedVocab, setDisplayedVocab] = useState(vocabData);
    ...
    useEffect(() => {
        setDisplayedVocab(isShuffled ? shuffleArray(vocabData) : vocabData);
        setCurrentCard(0);
        setIsFlipped(false);
        setUnlearnedWords([]);
        setIsStudyFinished(false);
    }, [isShuffled, vocabData]);
                ...
                <div className={`flashcard-container ${isFlipped ? 'flipped' : ''}`} onClick={handleFlip}>
                    <div className="flashcard">
                        {isReversed ? (
                            <>
                                <div className="front">
                                    <h2>{currentVocab.nghia}</h2>
                                </div>
                                <div className="back">
                                    <h2 className="kanji">{currentVocab.kanji}</h2>
                                    <h2 className="tu_vung">{currentVocab.tu_vung}</h2>
                                </div>
                            </>
                        ) : (
                            <>
                        ...
                        <button className="speak-button" onClick={(e) => speak(currentVocab.tu_vung, e)}>
                            <FaVolumeUp />
                        </button>
                ...
                <div className="unlearned-words">
                    ...
                    <ul>
                        {unlearnedWords.map((word, index) => (
                            <li key={index}>{word.tu_vung} - {word.kanji} - {word.nghia}</li>
                        ))}
                    </ul>
                </div>
            )}
